...
from authentication.models import User, History, Company, CompanyStuff, SecurityQuestion, SecurityAnswer, Client, \
    SwivelToken, PreferLocation
...
class SwivelTokenAdmin(admin.ModelAdmin):
    model = SwivelToken
    list_display = ['key', 'user', 'client']


admin.site.register(SwivelToken, SwivelTokenAdmin)


class PreferLocationAdmin(admin.ModelAdmin):
    model = PreferLocation
    list_display = ['id', 'client', 'country', 'state',  'city', 'address', 'postal_code', 'longitude', 'latitude']


admin.site.register(PreferLocation, PreferLocationAdmin)
