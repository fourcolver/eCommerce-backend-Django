...
from .models import User, Company, CompanyStuff, History, SecurityAnswer, SecurityQuestion, Client, SwivelToken, \
    PreferLocation
from .serializers import UserSerializer, CompanySerializer, SecurityAnswerSerializer, \
    SecurityQuestionSerializer, StandardSecurityAnswerSerializer, BasicAuthenticationSerializer,\
    BasicClientAuthenticationSerializer, PreferLocationSerializer, ClientSerializer
...
class ClientCreateView(generics.CreateAPIView):
    ...
    """
    serializer_class = settings.SERIALIZERS.client_create
    permission_classes = [permissions.AllowAny]
    ...
    def perform_create(self, serializer):
        ...
        SwivelToken.objects.create(client=user)
        PreferLocation.objects.create(client=user)
        context = {'user': user}
...
class UserCreateView(generics.CreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        SwivelToken.objects.create(user=user)
...
class VendorAuth(ObtainAuthToken):
    ...
    def post(self, request, *args, **kwargs):
        ...
        token = SwivelToken.objects.get(user=user)
        if os.getenv('DB_NAME') != 'dropify':
            get_client_ip(request, user)
        return Response({'token': token.key, 'status': user.status})


class ClientAuth(ObtainAuthToken):
    serializer_class = BasicClientAuthenticationSerializer
    permission_classes = (AllowAny,)
    ...
    def post(self, request, *args, **kwargs):
        ...
        serializer.is_valid(raise_exception=True)
        user = serializer.validated_data['user']
        token = SwivelToken.objects.get(client=user)
        user_serializer = ClientSerializer(user)
        return Response({'token': token.key, 'user': user_serializer.data})


def get_client_ip(request, user):
...
class ClientActivationView(utils.ActionViewMixin, generics.GenericAPIView):
    """
    Use this endpoint to activate user account.
    """
    serializer_class = settings.SERIALIZERS.activation_client
    permission_classes = [permissions.AllowAny]
    token_generator = default_token_generator
...
class PreferLocationUpdateView(APIView):
    serializer_class = PreferLocationSerializer

    def get(self, *args, **kwargs):
        user = self.request.user
        prefer_location = PreferLocation.objects.filter(client=user).first()
        if prefer_location is None:
            raise Http404
        pl_serializer = PreferLocationSerializer(prefer_location)
        return Response(pl_serializer.data)
    ...
    def put(self, *args, **kwargs):
        ...
        if prefer_location is None:
            raise Http404
        self.request.data['client'] = user.id
        pl_serializer = PreferLocationSerializer(prefer_location, self.request.data)
