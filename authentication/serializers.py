...
from .models import User, Company, CompanyStuff, SecurityQuestion, SecurityAnswer, Client, PreferLocation
# User = get_user_model()
...
class ClientSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Client
        fields = tuple(User.REQUIRED_FIELDS) + (
            Client._meta.pk.name,
            Client.USERNAME_FIELD, 'surname', 'name', 'phone', 'phone_verified', 'country', 'state',  'city', 'address',
            'postal_code', 'avatar', 'birthday', 'gender', 'status'
        )
        read_only_fields = (Client.USERNAME_FIELD,)

    def update(self, instance, validated_data):
        email_field = get_user_email_field_name(Client)
        if settings.SEND_ACTIVATION_EMAIL and email_field in validated_data:
            instance_email = get_user_email(instance)
            if instance_email != validated_data[email_field]:
                instance.is_active = False
...
class UserCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = tuple(User.REQUIRED_FIELDS) + (
            User.USERNAME_FIELD, User._meta.pk.name, 'surname', 'name', 'phone', 'phone_verified', 'country', 'state','is_customer',
...
class ClientCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Client
        fields = tuple(User.REQUIRED_FIELDS) + (
            Client.USERNAME_FIELD, Client._meta.pk.name, 'surname', 'name', 'phone', 'phone_verified', 'country', 'state','is_customer',
            'password', 'city', 'address', 'postal_code', 'avatar', 'birthday', 'gender', 'status'
        )
...
class ClientUidAndTokenSerializer(serializers.Serializer):
    uid = serializers.CharField()
    token = serializers.CharField()
    ...
    def validate_uid(self, value):
        try:
            uid = utils.decode_uid(value)
            self.user = Client.objects.get(pk=uid)
    ...
    def validate(self, attrs):
        attrs = super(ClientUidAndTokenSerializer, self).validate(attrs)
        is_token_valid = self.context['view'].token_generator.check_token(
            self.user, attrs['token']
        )
        if is_token_valid:
...
class ClientActivationSerializer(ClientUidAndTokenSerializer):
    default_error_messages = {'stale_token': constants.STALE_TOKEN_ERROR}

    def validate(self, attrs):
        attrs = super(ClientActivationSerializer, self).validate(attrs)
        if not self.user.is_active:
            return attrs
        raise exceptions.PermissionDenied(self.error_messages['stale_token'])
...
class JSONWebTokenSerializer(Serializer):
    ...
    def validate(self, attrs):
        # check store
        if 'is_customer' in self.initial_data:
            is_customer = self.initial_data['is_customer']
        else:
            is_customer = False
        if is_customer:
            credentials = {
...
class PreferLocationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('id', 'client', 'country', 'state',  'city', 'address', 'postal_code',
                  'longitude', 'latitude')
...
class BasicAuthenticationSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if email and password:
            ...
            if user:
                if not user.is_active:
                    msg = _('User account is disabled, please verify your email')
                    raise serializers.ValidationError(msg, code='authorization')
            else:
                # user = get_object_or_404(models.User,username=email)
...
class BasicClientAuthenticationSerializer(serializers.Serializer):
    email = serializers.CharField()
    password = serializers.CharField(style={'input_type': 'password'})
